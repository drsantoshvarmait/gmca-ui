...
export default function OrganisationManagement() {
  ...
  async function loadData() {
    ...
    try {
      ...
      // 2. Load Tenants for the dropdown
  ...
  async function handleAddOrg(e) {
    ...
    const insertData = {
        ...
        tenant_id: newOrg.tenant_id || null
    }
    ...
    if (error) {
      ...
      setNewOrg({ organisation_name: "", tenant_id: tenants[0]?.tenant_id || "" })
    }
  }
          ...
          <table style={table}>
            <thead>
              <tr style={tableHeader}>
                <th style={th}>ID</th>
                <th style={th}>Name</th>
                <th style={th}>Tenant Association</th>
              </tr>
            </thead>
            <tbody>
              {orgs.length === 0 ? (
                <tr>
                  <td colSpan="3" style={emptyCell}>No organisations found.</td>
                </tr>
              ) : (
                orgs.map((o) => (
                  <tr key={o.organisation_id} style={tr}>
                    ...
                    <td style={td}>{o.tenant_id || <span style={{color: '#9ca3af'}}>Shared/Global</span>}</td>
                  </tr>
                ))
              )}
            </tbody>
      ...
      {showAddModal && (
        <div style={modalOverlay}>
          <div style={modalContent}>
            <h3 style={modalTitle}>New Organisation</h3>
            <form onSubmit={handleAddOrg} style={form}>
              ...
              <div style={inputGroup}>
                ...
                <p style={helpText}>Link this organisation to a specific department or tenant.</p>
              </div>
