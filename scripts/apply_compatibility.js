...
-- 1. Ensure core schema
CREATE SCHEMA IF NOT EXISTS core;

-- 2. Master Tenants (core)
...
-- Ensure all columns in core.tenants
...
-- 4. Public Tenants (compatibility table)
DO $$ BEGIN
...
-- Ensure all columns in public.tenants
DO $$ BEGIN ALTER TABLE public.tenants ADD COLUMN tenant_type TEXT DEFAULT 'GOVERNMENT'; EXCEPTION WHEN duplicate_column THEN NULL; END $$;
DO $$ BEGIN ALTER TABLE public.tenants ADD COLUMN settings JSONB DEFAULT '{}'::jsonb; EXCEPTION WHEN duplicate_column THEN NULL; END $$;

-- 5. Sync triggers for tenants
...
-- 7. Communication Schema (Partial)
...
-- Notifications View (Dummy for now to prevent crash)
DROP VIEW IF EXISTS public.v_notifications;
CREATE OR REPLACE VIEW public.v_notifications AS 
SELECT 
    id, 
    user_id, 
    COALESCE(type, 'Notification') as title, 
    message, 
    read as is_read, 
    created_at 
FROM core.notifications;

-- Workflow Health View (Dummy for now)
...
ALTER TABLE public.comm_subject ENABLE ROW LEVEL SECURITY;
...
DO $$ BEGIN
    ...
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Allow comm access' AND tablename = 'comm_subject') THEN
        CREATE POLICY "Allow comm access" ON public.comm_subject FOR ALL USING (true);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Allow letter access' AND tablename = 'letters') THEN
        CREATE POLICY "Allow letter access" ON public.letters FOR ALL USING (true);
    END IF;
END $$;
...
ON CONFLICT (organisation_type) DO NOTHING;

-- 11. Refresh PostgREST
...
async function main() {
    ...
    try {
        ...
        console.log("Applying enhanced compatibility layer...");
