...
async function resetEnv(config, name) {
    const client = new Client(config);
    try {
        await client.connect();
        console.log(`Wiping ${name} environment for a clean rebuild...`);
        await client.query(clearSql);
        console.log(`✅ ${name} environment cleared.`);
    } catch (err) {
...
async function main() {
    ...
    if (env === 'staging') {
        await resetEnv(STAGING, 'STAGING');
    } else if (env === 'prod') {
        await resetEnv(PROD, 'PRODUCTION');
    } else {
        console.log("Usage: node scripts/clear_environment.js [staging|prod]");
    }
}
